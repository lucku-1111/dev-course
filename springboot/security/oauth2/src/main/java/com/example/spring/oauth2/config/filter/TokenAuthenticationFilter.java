package com.example.spring.oauth2.config.filter;


import com.example.spring.oauth2.config.jwt.TokenProvider;
...
// 4) 토큰이 없거나 무효면 인증없이 통과 -> 보호된 경로라면 체인 끝의 AuthorizationFilter가 거부(401)
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (token != null) {
            ...
            if (status == TokenStatus.VALID) {
                ...
                // SecurityContextHolder...getAuthentication().getPricipal()로 꺼낼 수 있다.
