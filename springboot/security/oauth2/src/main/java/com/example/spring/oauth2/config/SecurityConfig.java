...
import com.example.spring.oauth2.config.filter.TokenAuthenticationFilter;
...
// -> /oauth2/authorization/{registerationId} 요청을 가로채 인가 페이지로 리다이렉트
// 2) OAuth2LoginAuthenticationFilter
// -> /login/oauth2/code/{registerationId}로 돌아온 code를 받아
...
// 정리하자면 개발자가 구현하는 것은 파이프라인의 양 끝 훅(hook) 두 개뿐이다.
// - OAuth2UserService: 제공자 응답 -> 우리 회원 매핑
// - SuccessHandler: 로그인 성공 -> 후처리(토큰 발급, 리다이렉트 등)
// 나머지(리다이렉트, state, 코드-토큰 교환, 정보 조회)는 전부 프레임워크가 처리하고,
