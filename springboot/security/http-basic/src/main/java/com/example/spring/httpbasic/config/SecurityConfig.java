...
// DelegatingFilterProxy 자신은 평범한 서플릿 필터라서 톰캣에 정상적으로 등록할 수 있다.(톰캣 입장에선 그냥 일반 필터 중 하나)
...
public class SecurityConfig {
    ...
    // 이 Bean의 역할은 "인증할 떄 사용자 정보를 어디서 가져올지"를 정의한다.
    // AuthenticationManager가 아이디로 사용자를 조회할 때 바로 이 UserDetailsService를 호출한다.
    @Bean
    ...
    // 이 Bean의 역할은 "비밀번호를 어떻게 비교/암호화 할지"를 정의한다.
